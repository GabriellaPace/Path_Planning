...
for sample in range(1):  #for sample in range(40):
  ...
  sobelx = cv2.Sobel(out_image, cv2.CV_32F, 1, 0,       ksize=3, scale=0.23,        borderType=cv2.BORDER_ISOLATED)   #scale=0.25
  sobely = cv2.Sobel(out_image, cv2.CV_32F, 0, 1,       ksize=3, scale=0.23,        borderType=cv2.BORDER_ISOLATED)   #scale=0.25
  ...
  if blur:
    ...
    blur_img = cv2.blur(~slope, (height//4,height//4))
  ...
  if blur:
    cv2.imwrite(f"{path}/{sample}_gradient_BLUR.bmp", blur_img)